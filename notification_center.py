...
SCRIPT_LICENSE = 'MIT'
SCRIPT_DESC = ('Pass highlights and private messages to '
               'the OS X 10.8+ Notification Center')
...
def notify(data, buffer, date, tags, displayed, highlight, prefix, message):
    ...
    # passing `None` or `''` still plays the default sound so we pass a
    # lambda instead
    ...
    if weechat.config_get_plugin('show_highlights') == 'on' and int(highlight):
        ...
        weechat.config_get_plugin('show_private_message') == 'on' and
        'notify_private' in tags
    ):
        if weechat.config_get_plugin('show_message_text') == 'on':
            Notifier.notify(
                message,
